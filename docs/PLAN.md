...
// 패키지 접근 또는 별도 호출용
ProductionQueueItem createQueueItem(Order order, Sample sample)
...
void approveOrder()  // RESERVED 목록 표시 → 선택 → 재고 비교
                     ...
                     //   재고 부족: 상태 PRODUCING, productionLineController.createQueueItem() 호출 후 enqueue
...
ConsoleMVC의 `Router`를 5개 메뉴로 확장한다.
